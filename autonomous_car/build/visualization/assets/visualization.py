...
# Xoá path plugin mà cv2 tự set, tránh dùng plugin của OpenCV
os.environ.pop("QT_QPA_PLATFORM_PLUGIN_PATH", None)

# Ép Qt dùng xcb (trên Ubuntu có GUI)
...
from assets.FACE_DETECTION.face import FaceRecognition
...
pkg_share = get_package_share_directory('visualization')
...
class MapDisplayFrame(QFrame):
    ...
    def ms_clicked(self):
        ...
        msg = String()
        msg.data = "MS"
        self.parent().node.route_pub.publish(msg)
...
class AutonomousCarUI(QWidget):
    def __init__(self):
        ...
        self.subscription = self.node.create_subscription(
            Image,
            '/camera/image_raw',   # topic bạn đang publish
    ...
    def listener_callback(self, msg):
        self.node.get_logger().info('\tImage set!')
        global image_global
        # Convert ROS Image -> OpenCV
        frame = self.bridge.imgmsg_to_cv2(msg, desired_encoding='bgr8')

        # xử lý frame ở đây
        print("Nhận được frame có kích thước:", frame.shape)

        image_global = frame

        # nếu muốn xem thì mở imshow
