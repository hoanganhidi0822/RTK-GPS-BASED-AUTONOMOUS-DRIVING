...
class MapDisplayFrame(QFrame):
    ...
    def switch_to_ros_camera(self):
        ...
        # dừng face detection (vì face detection dùng cam trực tiếp)
        if hasattr(self, "face_timer") and self.face_timer.isActive():
            self.face_timer.stop()
...
class AutonomousCarUI(QWidget):
    def __init__(self):
        ...
        self.gps_fix_sub = self.node.create_subscription(
            NavSatFix,
            '/gps/fix',
        ...
        self.gps_heading_sub = self.node.create_subscription(
            Float32,
            '/gps/heading',
        ...
        self.gps_speed_sub = self.node.create_subscription(
            Float32,
            '/gps/speed',
    ...
    def gps_fix_callback(self, msg: NavSatFix):
        ...
        # Nếu đã có heading thì convert luôn sang x, y, yaw
        if self.current_heading_deg is not None:
            self.update_xy_yaw_from_gps()

    def gps_heading_callback(self, msg: Float32):
        self.current_heading_deg = msg.data

        # Nếu đã có lat/lon thì convert luôn sang x, y, yaw
        if self.current_lat is not None and self.current_lon is not None:
            self.update_xy_yaw_from_gps()

    def gps_speed_callback(self, msg: Float32):
    ...
    def update_xy_yaw_from_gps(self):
        """
        Convert (lat, lon, heading_deg) -> (x, y, yaw) dùng cùng hàm như main.
        """
        try:
