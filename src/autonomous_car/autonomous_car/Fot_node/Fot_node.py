...
WAYPOINT_FILE = str(pathlib.Path(__file__).resolve().parents[2] / "way" / "merged_waypoints.txt")
SPEED_UNIT = "mps"    
TIMER_DT = 1/20     
DBG_EVERY = 10
...
def smoothed_yaw_rad(new_yaw_rad: float) -> float:
    yaw_buffer.append(float(new_yaw_rad))
...
class Fot_node(Node):
    def __init__(self):
        ...
        self.get_logger().info(f"CFG: {WAYPOINT_FILE} | timer={1/TIMER_DT:.1f}Hz")
    ...
    def publish_reference_path(self):
        ...
        self._ref_pub_count += 1
        if (self._ref_pub_count % 20) == 0:
            self.get_logger().info(
                f"[DEBUG] Published /frenet/reference_path with {len(path_msg.poses)} poses"
            )
    ...
    def publish_optimal_path(self, fp):
        ...
        self._opt_pub_count += 1
        if (self._opt_pub_count % 20) == 0:
            self.get_logger().info(
                f"[DEBUG] Published /frenet/optimal_path with {len(path_msg.poses)} poses"
            )
    ...
    def timer_callback(self):
        if (not self.ready) or (self.csp is None):
            return
        if (not self.have_odom) or (self.current_yaw is None):
            return

        self._tick += 1

        # 1) yaw smooth
        yaw = smoothed_yaw_rad(self.current_yaw)
